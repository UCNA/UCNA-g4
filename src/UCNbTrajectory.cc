...
#include "UCNbTrajectoryPoint.hh"
...
G4Allocator<UCNbTrajectory> UCNbTrajectoryAllocator;
...
UCNbTrajectory::UCNbTrajectory(const G4Track* aTrack)
{
    ...
    fpPointsContainer = new UCNbTrajectoryPointContainer();
    // Following is for the first trajectory point
    fpPointsContainer->push_back(new UCNbTrajectoryPoint(aTrack));
...
UCNbTrajectory::UCNbTrajectory(UCNbTrajectory & right) : G4VTrajectory()
{
    ...
    for(size_t i=0;i<right.fpPointsContainer->size();++i) {
        UCNbTrajectoryPoint* rightPoint
            = (UCNbTrajectoryPoint*)((*(right.fpPointsContainer))[i]);
        fpPointsContainer->push_back(new UCNbTrajectoryPoint(*rightPoint));
    }
...
void UCNbTrajectory::ShowTrajectory(std::ostream& os) const
{
...
void UCNbTrajectory::DrawTrajectory(G4int i_mode) const
{
    // Invoke the default implementation in G4VTrajectory...
    ...
    if (lineRequired) {
      ...
      if(particleDefinition==G4OpticalPhoton::OpticalPhotonDefinition()){
        if(wls) // WLS photons are 
          colour = G4Colour(0.4,0.9,0.2);
        else{ //Scintillation and Cerenkov photons are 
          colour = G4Colour(0.,1.,1.);
        }
      }
      else //All other particles are 
        colour = G4Colour(0.0,0.0,1.);
...
void UCNbTrajectory::AppendStep(const G4Step* aStep)
{
    fpPointsContainer->push_back(new UCNbTrajectoryPoint(aStep));
}

G4ParticleDefinition* UCNbTrajectory::GetParticleDefinition()
{
    return (G4ParticleTable::GetParticleTable()->FindParticle(ParticleName));
}

void UCNbTrajectory::MergeTrajectory(G4VTrajectory* secondTrajectory)
{
    if(!secondTrajectory) return;

    UCNbTrajectory* second = (UCNbTrajectory*)secondTrajectory;
...
const std::map<G4String,G4AttDef>* UCNbTrajectory::GetAttDefs() const
{
    G4bool isNew;
...
std::vector<G4AttValue>* UCNbTrajectory::CreateAttValues() const
{
